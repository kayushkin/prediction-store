...
// 2026-09-20: ":8313" and the default data directory with nothing set, and the
...
func TestTheRegistryReadsTheSameValuesTheCommandAlwaysDid(t *testing.T) {
	...
	if got := unset.String(SettingListenAddress); got != ":8313" {
		t.Errorf("listen address with nothing set = %q, want :8313", got)
	}
	...
	set, err := NewSettingsRegistry(servicesettings.MapEnvironment(map[string]string{
		"PREDICTION_STORE_ADDR":     "127.0.0.1:9999",
		"PREDICTION_STORE_DATA_DIR": "/srv/predictions",
	}))
	if err != nil {
		t.Fatal(err)
	}
	...
	if got := set.String(SettingDataDirectory); got != "/srv/predictions" {
	...
	empty, err := NewSettingsRegistry(servicesettings.MapEnvironment(map[string]string{"PREDICTION_STORE_ADDR": "", "PREDICTION_STORE_DATA_DIR": ""}))
	if err != nil {
		t.Fatal(err)
	}
	if empty.String(SettingListenAddress) != ":8313" || empty.String(SettingDataDirectory) != DefaultDataDir() {
		t.Errorf("empty variables: listen=%q data=%q", empty.String(SettingListenAddress), empty.String(SettingDataDirectory))
	}
}

func TestTheRegistryRefusesAPredictionStoreVariableNobodyDeclared(t *testing.T) {
	_, err := NewSettingsRegistry(servicesettings.MapEnvironment(map[string]string{"PREDICTION_STORE_ADDRESS": ":8313"}))
	if err == nil || !strings.Contains(err.Error(), "PREDICTION_STORE_ADDRESS is set and prediction-store declares no such setting") {
		t.Fatalf("NewSettingsRegistry = %v, want a refusal naming the misspelled variable", err)
	}
	if _, err := NewSettingsRegistry(servicesettings.MapEnvironment(map[string]string{"PREDICTION_STORE_ADDR": ":1", "PATH": "/bin", "HOME": "/root"})); err != nil {
...
func TestGetSettingsDescribesTheServiceAndNothingCanBeWritten(t *testing.T) {
	registry, err := NewSettingsRegistry(servicesettings.MapEnvironment(map[string]string{"PREDICTION_STORE_ADDR": ":9999"}))
	if err != nil {
		t.Fatal(err)
	}
...
func TestEveryEnvironmentVariableTheServiceReadsIsDeclared(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join("cmd", "prediction-store", "main.go")); err != nil {
