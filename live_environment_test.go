...
//
//	go test -tags sqlite_fts5 -run TestTheLiveProcessEnvironmentBuildsARegistry . -args -live-environment-file=/proc/<pid>/environ
//
// deploy.sh runs it against the running service before it stops it, because a
// set PREDICTION_STORE_ variable that nobody declared stops the new binary at boot.
